...
### 1.1.1. 客户端缓存：浏览器、app等

见1.2.1

### 1.1.2. CDN缓存

用于缓存静态数据，如cloudflare、七牛云等
...
nginx通过返回的`Expires`，`Cache-control`等头部字段告诉浏览器是否要缓存及过期的时间；并通过浏览器请求时的`Etag`头部字段判断是返回304，还是重新请求字段
 ...
 ![img](../imgs/1165731-20190623163949263-298746206.png)
...
概念：在缓存系统中，大量的缓存数据在同一时间失效或者被清空，导致大量的请求直接打到底层数据库或服务，从而引起数据库负载剧增

产生原因：1、同时失效或清空；2、缓存服务宕机；3、热点数据缓存失效
...
- Cache-Aside Pattern：旁路缓存模式，常用，适用于读多写少的场景。读操作时，检查缓存中有则返回，没有则查后端数据库写缓存并返回；写操作时，修改后端数据库并删除缓存。
...
缓存的容量是有限的，当缓存容量达到上限时，需要采取合适的淘汰策略来删除一些数据。常见的淘汰策略包括LRU（最近最少使用）、LFU（最不经常使用）和FIFO（先进先出）等

### 1.2.6. 缓存安全性

缓存中可能存储敏感数据，需要采取相应的安全措施，如数据加密、访问控制和防止缓存投毒攻击等

### 1.2.7. 缓存性能监控

需要监控缓存的命中率、缓存大小、缓存访问延迟等指标，以及及时调整缓存配置和容量，以保证缓存的性能和可用性
...
1. 手动更新：开发人员手动更新缓存，适用于数据变更频率较低或需要精确控制缓存更新的场景。
2. 定时更新：通过定时任务或定时器触发，定期更新缓存数据，适用于数据变更频率较高但实时性要求不高的场景。
3. 延时更新：在数据变更后，延迟一段时间再更新缓存，以避免瞬时大量的缓存更新操作，适用于数据变更频繁且对实时性要求较高的场景。
4. 增量更新：仅更新变更的部分数据，而不是全部数据，以减少缓存更新的开销，适用于数据变更频繁但变更范围较小的场景。
5. 异步更新：在数据变更后，通过消息队列或事件触发机制，将缓存更新操作异步执行，以避免阻塞主线程，适用于对实时性要求较高的场景。
6. 缓存失效策略：设置缓存的过期时间或失效机制，当缓存过期或达到一定条件时，触发缓存更新操作，适用于需要自动刷新缓存的场景。
