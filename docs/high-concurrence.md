...
总结：1、一次获取所有资源；2、获取资源后尽快释放；3、如果需要等待另一个资源，则释放所有资源，稍后重试
...
Paxos算法是分布式系统中用于实现一致性的算法，由Leslie Lamport于1998年提出。它是一种基于消息传递的一致性算法，可以在分布式系统中确保多个节点之间达成一致的共识。

Paxos算法解决的问题是分布式系统中的共识问题，即如何让多个节点就某个提案达成一致的决定。这个问题在分布式系统中非常重要，例如在分布式数据库中的数据一致性、分布式事务中的协调、分布式锁的实现等场景都需要解决共识问题。

Paxos算法的核心思想是通过消息传递来实现多个节点之间的协调。它分为三个阶段：

1. **准备阶段（Prepare Phase）**：提议者（Proposer）向多数派的节点发送提案编号（Proposal Number）的请求。节点收到请求后，如果提案编号大于它已经回复的任何请求的提案编号，则向提议者回复自己已接受的最高提案编号和对应的值；否则，拒绝请求。
2. **承诺阶段（Promise Phase）**：如果提议者收到了多数派节点的回复，且所有回复中的提案编号都不小于当前提案的编号，则提议者会发送接受请求，表示自己接受该提案。节点在接受请求后，如果没有接受其他提案，则接受该提案。
3. **学习阶段（Learn Phase）**：一旦提议者收到了多数派节点的接受请求，它就可以向所有节点广播提案，并将其作为已达成的共识。

Paxos算法通过这三个阶段的协议，保证了在任何时刻只有一个提案被接受，并且只有提案编号最大的提案才能被接受。这样就实现了分布式系统中的共识。

### 1.4.2. Raft协议

Raft协议是一种分布式一致性协议，它与Paxos算法类似，用于解决分布式系统中的共识问题。Raft协议由Diego Ongaro和John Ousterhout于2013年提出，旨在提供更易理解、更易实现和更易调试的一致性算法。

Raft协议的设计主要围绕三个核心问题：领导选举、日志复制和安全性。它将分布式系统中的节点划分为三种角色：领导者（Leader）、跟随者（Follower）和候选者（Candidate）。

Raft协议的主要特点包括：

1. **领导者选举（Leader Election）**：Raft使用随机化的超时机制来触发领导者选举。当一个跟随者在一段时间内没有接收到领导者的消息时，它会变成候选者并开始一轮新的选举。候选者向其他节点发送投票请求，得到多数投票后成为新的领导者。
2. **日志复制（Log Replication）**：领导者负责接收客户端的请求，并将这些请求附加到自己的日志条目中。一旦日志条目被大多数节点确认接收，领导者就会将这些条目复制到其他节点的日志中。这样，所有节点的日志最终保持一致。
3. **安全性**：Raft协议保证了在任何时间点都只有一个领导者，并且只有领导者才能接受客户端的请求。此外，Raft还使用递增的日志索引和任期号来保证节点之间的通信安全性。
4. **易理解性**：相对于Paxos算法，Raft协议的设计更加直观和易于理解。它采用了更简洁的算法流程，将整个一致性过程分解为几个清晰的阶段，使得开发人员更容易理解和实现。

总的来说，Raft协议通过领导者选举和日志复制等机制，提供了一种简单且可靠的分布式一致性解决方案，适用于各种分布式系统的场景。
...
1、共享锁、排他锁、互斥锁：多个线程可同时获取共享锁，但只能有一个线程能获取排他锁、互斥锁

2、乐观锁、悲观锁：上面的共享锁、排他锁都是悲观锁，乐观锁不是真正的锁，是一种并发控制的思想
