...
- 触发时机：开启后根据save配置周期写入；服务停止时写入（有修改）；或者手动执行命令写入。启动时加载
- 命令：`bgsave`

默认save配置：60s内有1000次修改，300s内有10s修改，900s内有1次修改
...
### 1.9.2. 生成快照 SAVE/BGSAVE

### 1.9.3. AOF重写 BGREWRITEAOF
...
跳跃表（Skip List）是一种有序数据结构，用于在数据集中快速查找元素。跳跃表通过添加多层索引来加速查找操作，从而在平均情况下实现 O(log n) 的查找时间复杂度，类似于平衡树（如红黑树），但实现更为简单。

跳跃表的基本思想是在底层维护一个有序链表，同时在链表上方建立多级索引，每一级索引都是原链表的一部分。每一级索引都是原链表的一种“跳跃”，使得在查找时可以跳过一定数量的节点，从而快速地定位到目标节点。最顶层的索引只有两个节点，分别指向整个链表的头和尾，这样可以在最短的时间内定位到目标节点。
